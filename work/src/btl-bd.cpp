...
#ifdef TREE_ADAM
  tree_m_.resize(256 * hsize_, 0.0f);
  tree_v_.resize(256 * hsize_, 0.0f);
  tree_t_ = 0;
#endif
...
int BtLstm::ExpectedByte(int c0) const {
  int n = c0;
  if (n < 1) n = 1;
  while (n < 256) {
    const int bit = (PBit(n) >= 0.5f) ? 1 : 0;
    n = (n << 1) | bit;
  }
  return n & 255;
}
...
#ifdef TREE_ADAM
  float* m = &tree_m_[(size_t)c0 * hsize_];
  float* v = &tree_v_[(size_t)c0 * hsize_];
  tree_t_ += 1.f;
  const float b1 = 0.9f, b2 = 0.999f, eps = 1e-8f;
  const float inv1 = 1.f / (1.f - powf(b1, tree_t_));
  const float inv2 = 1.f / (1.f - powf(b2, tree_t_));
  for (unsigned int j = 0; j < hsize_; ++j) {
    row[j] = w[j];
    const float g = d * hidden_[j];
    m[j] = b1 * m[j] + (1.f - b1) * g;
    v[j] = b2 * v[j] + (1.f - b2) * g * g;
    w[j] -= learning_rate_ * (m[j] * inv1) / (sqrtf(v[j] * inv2) + eps);
  }
#else
