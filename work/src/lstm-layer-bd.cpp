float g_initMul = 0.5f;
int g_blocks = 4;   // block-diagonal recurrent weights
#ifndef FORGET_BIAS
#define FORGET_BIAS 1.0f
#endif
float g_forgetBias = FORGET_BIAS;  // last-weight forget-gate bias (cmix default = 1)
...
LstmLayer::LstmLayer(unsigned int input_size, unsigned int auxiliary_input_size,
  ...
  { extern float g_initMul; val *= g_initMul; }
  float low = -val;
  float range = 2 * val;
  for (unsigned int i = 0; i < num_cells_; ++i) {
    ...
    forget_gate_.weights_[i][forget_gate_.weights_[i].size() - 1] = g_forgetBias;
  }
  MaskAll();
}
...
static void MaskBlocks(NeuronLayer& neurons, unsigned int num_cells, unsigned int output_size){
  if(g_blocks<=1) return;
  const unsigned int blk = num_cells / g_blocks;
  for(unsigned int i=0;i<num_cells;i++){
    unsigned int b = (blk? i/blk : 0); if(b>= (unsigned)g_blocks) b=g_blocks-1;
    const unsigned int lo=b*blk, hi=lo+blk;
    for(unsigned int j=0;j<num_cells;j++)
      if(j<lo||j>=hi) neurons.weights_[i][output_size+j]=0.0f;
  }
}
void LstmLayer::MaskAll(){
  MaskBlocks(forget_gate_,num_cells_,output_size_);
  MaskBlocks(input_node_,num_cells_,output_size_);
  MaskBlocks(output_gate_,num_cells_,output_size_);
}
void LstmLayer::ForwardPass(NeuronLayer& neurons,
  ...
  if (g_blocks <= 1) {
    ...
    const unsigned int blk = num_cells_ / g_blocks;
    const unsigned int isz = input.size();
    for (unsigned int i = 0; i < num_cells_; ++i) {
      const float* w = &neurons.weights_[i][output_size_];
      float f = neurons.weights_[i][input_symbol];
      unsigned int b = i / blk; if (b >= (unsigned int)g_blocks) b = g_blocks - 1;
      const unsigned int lo = b * blk, hi = lo + blk;
      for (unsigned int j = lo; j < hi; ++j) f += input[j] * w[j];
      for (unsigned int j = num_cells_; j < isz; ++j) f += input[j] * w[j];
      neurons.norm_[epoch_][i] = f;
    }
  }
...
void LstmLayer::BackwardPass(NeuronLayer& neurons,
  ...
  if (g_blocks <= 1) {
    ...
    for (unsigned int i = 0; i < num_cells_; ++i) {
      ...
      float* u = &neurons.update_[i][output_size_];
      unsigned int b = i / blk; if (b >= (unsigned int)g_blocks) b = g_blocks - 1;
      const unsigned int lo = b * blk, hi = lo + blk;
      for (unsigned int j = lo; j < hi; ++j) u[j] += e * input[j];
      for (unsigned int j = num_cells_; j < isz; ++j) u[j] += e * input[j];
